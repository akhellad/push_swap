...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/17 16:23:26 by akhellad          #+#    #+#             */
/*   Updated: 2023/02/17 16:25:11 by akhellad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	bruteforce_order_a(t_init *init)
{
	...
	conds = new_empty_condition(&init->a_pile, NULL, init->a_pile.max_tab_size);
	if (!conds)
		return (1);
	result = pick_bruteforce_solution(conds, pos, &init->b_pile);
	if (!result)
	{
		free_condition(conds);
		return (1);
