...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/16 17:38:01 by akhellad          #+#    #+#             */
/*   Updated: 2023/02/16 17:42:12 by akhellad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pick_solution(t_init *init, t_cond *conds)
{
	...
	if (solution->dir)
	{
		...
		init->dir = copy_dir(solution->dir);
		if (!init->dir)
		{
...
int	resolve(t_init *init)
{
	...
	conds = new_empty_condition(&init->a_pile, \
		&init->b_pile, init->a_pile.max_tab_size);
	if (!conds)
		return (1);
